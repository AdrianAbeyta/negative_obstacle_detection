...
sensor_msgs::LaserScan VirtualFloorProjection( float virtual_floor_min_height_, float virtual_floor_max_height_, double y_prime_, const sensor_msgs::PointCloud2 cloud_filtered ){
    sensor_msgs::LaserScan  scan;
...
sensor_msgs::LaserScan PointToLaser( float point_to_laser_min_height_, float point_to_laser_max_height_ , const sensor_msgs::PointCloud2ConstPtr&  cloud_filtered ){
    ...
    //Option: Multi thread this for faster computation.
    for (sensor_msgs::PointCloud2ConstIterator<float> iter_x(*cloud_filtered, "x"),
    ...
    iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z)
    {
       ...
       if ( !std::isnan(*iter_x) && !std::isnan(*iter_y) && !std::isnan(*iter_z)) 
       {
            ...
            if (*iter_z > point_to_laser_max_height_ || *iter_z < point_to_laser_min_height_) 
            {
                continue;
            } 
