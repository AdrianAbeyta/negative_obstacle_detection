...
void pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr& camera_cloud, const sensor_msgs::PointCloud2ConstPtr& lidar_cloud){
    ...
    Filter_Camera_Points(pointcloud2_to_pcl, camera_cloud_filtered);
    
    // Filter the camrea points for desired useage (Furthest Point or Virtual Floor)
    FloorProjection( camera_cloud_filtered, floor_cloud_filter );
    NegativeLimitFilter( camera_cloud_filtered , negative_cloud_filter );
    ...
    B = VirtualFloorProjection( final_camera_points_B );
    
    //////////// OBSTACLE DETECTION ////////////

    // Get resdulting laserscan from filtered point clouds. 
    C = PointToLaser(lidar_cloud);
