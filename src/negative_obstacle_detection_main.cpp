...
FilterPCL* filter_cloud_;

using NegativeObstacle::PointCloudToLaser;
PointCloudToLaser* pointcloud_to_laser_;
...
void pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr& camera_cloud, const sensor_msgs::PointCloud2ConstPtr& lidar_cloud){
    ...
    pointcloud2_to_pcl = filter_cloud_->PointCloud2_To_PCL(camera_cloud);
    // Filter the raw camrea points 
    camera_cloud_filtered = filter_cloud_->Filter_Camera_Points(pointcloud2_to_pcl,camera_cloud_filtered);
    
    // Filter the camrea points for desired useage (Furthest Point or Virtual Floor)
    //floor_cloud_filter = filter_cloud_->FloorProjection( camera_cloud_filtered, floor_cloud_filter );
    negative_cloud_filter = filter_cloud_->NegativeLimitFilter( camera_cloud_filtered , negative_cloud_filter );
    ...
    B_ = pointcloud_to_laser_->VirtualFloorProjection( final_camera_points  );
    
    //////////// OBSTACLE DETECTION ////////////

    /////Points_To_Floor_filter = filter_cloud_->PointsToFloor(lidar_cloud, Points_To_Floor_filter ); // TODO: SET UP A WAY TO FILTER DOWN POINTCLOUDS
    
    // Get resdulting laserscan from filtered point clouds. 
    C_ = pointcloud_to_laser_->PointToLaser(lidar_cloud);
    

    // Combine the respective methods Laserscan into a single scan. 
    D_ = pointcloud_to_laser_->CombineLaserScans( B_ , C_ , D_);
